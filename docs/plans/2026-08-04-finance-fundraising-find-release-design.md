...
- [ ] `wave-6-criterion-3` **In progress:** Publish toward 5,000 useful resources without raw intake, synthetic seeds, or unverified discovery records — Evidence: after PR #174 passed quality and both deployments, the confirmed existing-only operation refreshed 25 unapproved Cook County staging rows, inserted zero records, added 803 field-evidence rows, and changed no review or public state. A new live check found the official source at 33 rows: Markham, Maywood, and Skokie courthouses were removed and LaGrange Park Public Library was added. Three of the first five canary records still match; two remain held. The publication guard now requires the latest verification result so a newer source removal supersedes historical approval. The public API remains at 853 resources with zero Cook County cooling centers; independent ledger storage, identified administrator review, and a maximum-five-record promotion canary remain.
