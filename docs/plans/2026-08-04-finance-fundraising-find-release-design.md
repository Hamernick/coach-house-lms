...
- [ ] `wave-6-criterion-3` **In progress:** Publish toward 5,000 useful resources without raw intake, synthetic seeds, or unverified discovery records — Evidence: the guarded Cook County cooling-center dry run found `34/35` current local records passing the publication contract, with one held, `25` matching existing unapproved staging rows, and nine not yet staged. A connected existing-only refresh plan selected exactly those 25 unapproved matches, skipped all nine unstaged rows, found zero protected matches, and made no write. The importer now fails closed against insertions, approved/ready/promoted matches, source drift, and refreshes above 25. Staging still lacks the refreshed fields, independent verification ledger, and identified administrator review; no publication candidate is eligible.
