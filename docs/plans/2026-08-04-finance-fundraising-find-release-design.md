...
- [ ] `wave-6-criterion-3` **In progress:** Publish toward 5,000 useful resources without raw intake, synthetic seeds, or unverified discovery records — Evidence: after PR #174 passed quality and both deployments, the confirmed existing-only operation refreshed 25 unapproved Cook County staging rows, inserted zero records, added 803 field-evidence rows, and changed no review or public state. PR #175 then merged with quality and both deployments green. Its bounded live verifier stored five deterministic results: Bremen Township, Brookfield Library, and Calumet Township passed against the current official source; removed Markham and Maywood courthouse rows were held. A guarded publication dry run selected zero records, proving the latest failed verification supersedes historical approval. The public API remains at 853 resources with zero Cook County cooling centers; identified administrator review and a maximum-five-record promotion canary remain.
