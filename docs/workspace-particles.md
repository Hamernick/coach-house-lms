...
- Particles is a tab in the existing workspace drawer. The catalog uses small preview tiles, search, and Roadmap / Drive / Images / Objectives filters.
...
- Drag a particle back onto the Particles tab/catalog or use Remove. Original sections, Drive attachments, and uploaded images remain available.
...
- Plan objective creates an editable objective with an optional yes/no decision, an action for each branch, shared steps, a completion checklist, tools/connections, and communication channels. The linked cards use the same positions, sizes, connections, permissions, and board autosave as other particles. Removing a card retains its plan in the catalog.
- Optional AI drafting produces a reviewable draft from the typed objective and notes only. It is disabled by default, unavailable in the sample preview, and blocked in production until durable organization usage limits exist. No model calls occurred during implementation. See [objective planner behavior and pilot limits](plans/2026-09-16-workspace-objective-planner.md).

## Ownership and review

Feature owners: `src/features/workspace-particles` and `src/features/workspace-objective-planner`. Route adapters compose them with the existing board; shared node-frame primitives own the visible card shell. Roadmap navigation and editor-save notifications provide the linked-section integration. The Drive Picker client reuses the existing implementation from the parallel Documents workstream.

The development-only `/visual-regression/workspace-particles` preview uses sample sections, a sample Drive reference, and browser-only fixture state. It cannot attach real files or upload images. Production `/workspace` uses the actual organization board and APIs. The preview is unavailable in production builds.

Tests cover board normalization, compatibility recovery, source IDs, private image authorization and validation, desktop drag/return, keyboard movement, sizes, linked refresh, read-only interaction, and image appearance. Browser checks also exercise touch dragging, connections, decision choices, action completion, plan editing, reload, and mobile planning. The separate Documents workstream records an earlier real Drive import/edit/save/reload and approved Google verification; this session has not repeated those checks. A real particle image upload and the current OAuth warnings remain unverified. See [Google verification and security review](plans/2026-09-16-google-oauth-security-review.md).
...
| 20 objective plans, 12 shared steps per plan | Bounds saved planning content | Planner and board normalizer |
