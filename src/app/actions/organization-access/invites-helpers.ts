...
export async function resolveInvitePermissionContext(
  ...
  const { orgId, role, isPlatformAdmin } =
    await resolveOrganizationActorContext(supabase, userId)
  ...
  if (!paidAccess.hasPaidTeamAccess)
    return { error: TEAM_ACCESS_UPGRADE_MESSAGE }

  const { adminsCanInvite } = await getOrganizationAccessSettings(
    supabase,
    orgId
  )
...
export function resolveOrganizationAccessNotificationTitle(
...
export async function resolveProfileSummaries(
  ...
  if (dedupedIds.length === 0)
    return new Map<string, { fullName: string | null; email: string | null }>()
...
function resolveInvitedMemberFullName({
  ...
  if (
    typeof invitedProfileName === "string" &&
    invitedProfileName.trim().length > 0
  ) {
    return invitedProfileName.trim()
  }
  if (
    typeof userMetadataName === "string" &&
    userMetadataName.trim().length > 0
  ) {
...
export async function ensureInvitedMemberInOrgDirectory({
  ...
  await mutateOrganizationPeopleProfile<Record<string, unknown>, null>({
