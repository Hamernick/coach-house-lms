    ...
    <header className={cn("text-muted-foreground flex shrink-0 flex-col bg-[var(--shell-bg)] text-sm", hideOnMobile && "max-md:hidden")}>
      ...
      >
        <div className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]">
          ...
          />
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-1 md:flex-nowrap md:gap-2 [&>button]:max-md:min-h-11 [&>button]:max-md:min-w-11">
            <div
              id="site-header-actions-right"
              className="flex flex-wrap items-center gap-2 md:flex-nowrap"
            />
            {hasUser && !isCompactMobileHeader && !(hideOnMobile && isMobile) ? (
