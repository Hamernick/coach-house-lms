...
export function DynamicForm({
  ...
  const defaultValues = Object.keys(schemaShape).reduce(
    (acc, key) => {
  ...
  useEffect(() => {
    if (initialValues) {
      ...
      const initialValuesRecord = initialValues as Record<string, unknown>
      const processedInitialValues = schemaKeys.reduce(
        (acc, key) => {
          ...
          const value = Object.prototype.hasOwnProperty.call(initialValuesRecord, key)
            ? initialValuesRecord[key]
            : undefined
      ...
      form.reset(processedInitialValues as Record<string, any>)
      setTimeout(() => {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)}>
        {Object.keys(schemaShape).map((fieldName) =>
          renderField(fieldName, schemaShape[fieldName])
        )}
        <div className="pt-6">
