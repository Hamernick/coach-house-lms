...
const CATEGORY_CHIP: Record<OrgPerson["category"], string> = Object.fromEntries(
  Object.entries(PERSON_CATEGORY_META).map(([key, value]) => [
    key,
    value.stripClass,
  ])
    ...
    <>
      <Item className="cursor-pointer" onClick={() => setEditing(true)}>
        <Avatar
          className={cn(
            "size-10",
            isSupporter && "bg-muted/60 ring-border/60 rounded-xl ring-1"
          )}
        >
          <AvatarImage
        ...
        <span
          className={`pointer-events-none absolute top-1/2 left-1.5 h-10 w-1 -translate-y-1/2 rounded-full sm:left-2 ${CATEGORY_CHIP[person.category]}`}
          aria-hidden="true"
        />
        <ItemActions>
          <DropdownMenu>
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will remove the person from
              your org chart and lists.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
