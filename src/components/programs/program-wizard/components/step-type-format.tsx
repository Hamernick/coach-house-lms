  ...
  return (
    <section className="grid gap-4 sm:grid-cols-2">
      <div className="grid gap-1.5">
        <Label>Public benefit focus</Label>
        <Select
          value={form.programType}
          onValueChange={(value) =>
            update({
              programType: value as ProgramWizardFormState["programType"],
            })
          }
        >
          <SelectTrigger>
        ...
        {errors.programType ? (
          <p className="text-destructive text-xs">{errors.programType}</p>
        ) : null}
      </div>
      <div className="grid gap-1.5">
        ...
        <Select
          ...
          onValueChange={(value) =>
            update({
              coreFormat: value as ProgramWizardFormState["coreFormat"],
            })
          }
        >
          <SelectTrigger>
        ...
        {errors.coreFormat ? (
          <p className="text-destructive text-xs">{errors.coreFormat}</p>
        ) : null}
      </div>
      <div className="sm:col-span-2">
        ...
        <div className="flex flex-wrap gap-2">
          {DELIVERY_FORMATS.filter((format) => format !== form.coreFormat).map(
            (format) => {
