...
const HIDDEN_HOME_SECTIONS = new Set<LegacyHomeSectionId>([
  "impact",
  "process",
  "news",
  "team",
...
export const CANVAS_NAV: CanvasNavItem[] = [
  ...LEGACY_HOME_SECTION_NAV.filter(
    (item) => !HIDDEN_HOME_SECTIONS.has(item.id)
  ).map((item) => ({
    ...item,
...
const HIDDEN_CANVAS_SECTION_IDS = new Set<CanvasSectionId>([
  "find",
  "login",
  "signup",
])
export const VISIBLE_CANVAS_NAV = CANVAS_NAV.filter(
  (item) => !HIDDEN_CANVAS_SECTION_IDS.has(item.id)
)

const HOME_SECTION_IDS = new Set<LegacyHomeSectionId>(
  LEGACY_HOME_SECTION_NAV.map((item) => item.id)
)
...
export function isHomeSectionId(
  sectionId: CanvasSectionId
): sectionId is LegacyHomeSectionId {
