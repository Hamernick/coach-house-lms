...
import {
  ...
  SidebarMenuButton,
  SidebarMenuItem,
} from "@/features/platform-admin-dashboard/upstream/components/ui/sidebar"
...
import { activeProjects, footerItems, navItems, type NavItemId, type SidebarFooterItemId } from "@/features/platform-admin-dashboard/upstream/lib/data/sidebar"
import {
  platformLabPath,
  stripPlatformLabBasePath,
} from "@/features/platform-admin-dashboard/upstream/lib/platform-lab-paths"
import { SettingsDialog } from "@/features/platform-admin-dashboard/upstream/components/settings/SettingsDialog"
import { AuthDialog, type AuthMode } from "@/features/platform-admin-dashboard/upstream/components/auth/AuthDialog"
...
export function AppSidebar() {
  ...
  const scopedPathname = stripPlatformLabBasePath(pathname)
  ...
  const getHrefForNavItem = (id: NavItemId): string => {
    if (id === "my-tasks") return platformLabPath("/tasks")
    if (id === "projects") return platformLabPath()
    if (id === "inbox") return platformLabPath("/inbox")
    if (id === "clients") return platformLabPath("/clients")
    if (id === "performance") return platformLabPath("/performance")
    return "#"
  }

  const isItemActive = (id: NavItemId): boolean => {
    if (id === "projects") {
      return scopedPathname === "/" || scopedPathname.startsWith("/projects")
    }
    if (id === "my-tasks") {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-800 text-primary-foreground shadow-[inset_0_-5px_6.6px_0_rgba(0,0,0,0.25)]">
              <img src="/platform-lab/logo-wrapper.png" alt="Logo" className="h-4 w-4" />
            </div>
          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <Avatar className="h-8 w-8">
                <AvatarImage src="/platform-lab/avatar-profile.jpg" />
