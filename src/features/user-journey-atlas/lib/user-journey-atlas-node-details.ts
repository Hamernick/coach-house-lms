...
> = {
  ...
  operations: {
    ...
    systemEvents: [
      "Closes the loop between product usage and operational follow-up",
    ],
    healthStatus: "integration-gap",
  },
}

export const USER_JOURNEY_NODE_DETAILS: Record<string, UserJourneyNodeDetails> =
  {
    public_home: {
      dataFields: ["auth callback params", "organization", "Find filters"],
      systemEvents: [
        "Mounts canonical Find and resumes pending auth redirects",
      ],
    },
    pricing_data: {
      dataFields: ["tier id", "CTA href", "feature copy"],
      systemEvents: [
        "Defines Individual, Organization, and Operations Support entry points",
      ],
    },
    home_canvas: {
      dataFields: ["section slug", "URL search params", "panel state"],
      systemEvents: [
        "Keeps signup and pricing interactions on the public canvas",
      ],
    },
    home_signup_panel: {
    ...
    sign_up_page: {
      ...
      systemEvents: [
        "Normalizes plan and redirects already-authenticated users",
      ],
    },
    sign_up_form: {
      dataFields: ["email", "password", "confirm password", "intent focus"],
      systemEvents: [
        "Sends account_intent, onboarding_intent_focus, and emailRedirectTo to Supabase",
      ],
    },
    confirmation_email_template: {
      ...
      healthSummary:
        "Needs visible resend, change-email, expired-link, and stalled-confirmation paths.",
    },
    auth_confirm: {
      dataFields: ["token_hash", "type", "next destination"],
      systemEvents: [
        "Verifies Supabase OTP and resolves safe redirect destination",
      ],
      healthStatus: "recovery-gap",
      healthSummary:
        "The verification route exists, but the atlas should show invalid/expired link recovery.",
    },
    ...
    stripe_checkout: {
      ...
      systemEvents: [
        "Creates Stripe session with user, org, plan, and subscription metadata",
      ],
      healthStatus: "recovery-gap",
      healthSummary:
        "Payment cancel, incomplete checkout, and login-return states need explicit recovery branches.",
    },
    pricing_success: {
      dataFields: ["session_id", "redirect", "context"],
      systemEvents: [
        "Verifies checkout and persists subscription state before redirect",
      ],
      healthStatus: "recovery-gap",
      healthSummary:
        "Checkout success needs a visible delayed-webhook and subscription-sync fallback path.",
    },
    onboarding_return: {
    ...
    stripe_webhook: {
      ...
      systemEvents: [
        "Receives signed Stripe events and reconciles subscription lifecycle",
      ],
    },
    onboarding_page: {
      dataFields: ["plan override", "default profile", "onboarding source"],
      systemEvents: [
        "Chooses paid plan override and renders post-signup setup",
      ],
    },
    onboarding_flow: {
    ...
    onboarding_pricing: {
      ...
      systemEvents: [
        "Starts Organization or Operations Support checkout from onboarding",
      ],
      healthStatus: "recovery-gap",
      healthSummary:
        "Needs a clear cancel/resume path when paid checkout interrupts onboarding.",
    },
    onboarding_org: {
      dataFields: ["organization name", "organization URL", "formation status"],
      systemEvents: [
        "Checks slug availability and prepares the workspace organization",
      ],
    },
    onboarding_account: {
      dataFields: [
      ...
      systemEvents: [
        "Carries profile data into account and public workspace context",
      ],
    },
    onboarding_card: {
      dataFields: [
    ...
    workspace_tutorial: {
      ...
      healthSummary:
        "The guide should write activation checkpoints and route users to a first real win.",
    },
    dynamic_form: {
      dataFields: [
        "schema fields",
        "initial values",
        "string/number/boolean/enum/array inputs",
      ],
    ...
    module_notes_api: {
      ...
      healthStatus: "activation-gap",
      healthSummary:
        "A saved note should trigger review, next-step guidance, or progress credit.",
    },
    assignment_form: {
      dataFields: [
        "short text",
    ...
    assignment_submission: {
      ...
      systemEvents: [
        "Sanitizes answers, stores submission, syncs mapped profile fields, and marks completion",
      ],
      healthStatus: "activation-gap",
      healthSummary:
        "Submitted homework needs feedback, coach review, and a visible next milestone.",
    },
    homework_assist: {
      dataFields: [
        "field name",
      ...
      systemEvents: [
        "Generates a homework suggestion from module and org profile context",
      ],
      healthStatus: "ai-stub",
      healthSummary:
        "The endpoint exists, but the generator is deterministic rather than model-backed.",
    },
    workspace_objective_editor: {
      ...
      healthStatus: "activation-gap",
      healthSummary:
        "Created objectives should become measurable tasks with ownership and reminders.",
    },
    workspace_calendar_form: {
      ...
      healthStatus: "integration-gap",
      healthSummary:
        "Internal events exist, but coach Google Calendar sync and booking confirmation are missing.",
    },
    workspace_comms_editor: {
      ...
      healthStatus: "integration-gap",
      healthSummary:
        "Scheduled communications are still simulated and need provider delivery states.",
    },
    org_profile_company: {
      dataFields: [
        "identity",
      ...
      healthStatus: "activation-gap",
      healthSummary:
        "Profile edits should close into completeness, publish, and Find visibility checkpoints.",
    },
    find_route: {
      dataFields: ["session", "member onboarding flag", "source"],
      systemEvents: [
        "Keeps public directory access safe for free and paid users",
      ],
    },
    public_map: {
    ...
    member_onboarding: {
      ...
      systemEvents: [
        "Introduces map, search, saved orgs, notifications, and organization switching",
      ],
      healthStatus: "activation-gap",
      healthSummary:
        "The free-member intro should record first saved org, contact, or follow-up intent.",
    },
    app_sidebar: {
    ...
    paywall_overlay: {
      ...
      healthStatus: "recovery-gap",
      healthSummary:
        "Upgrade dialogs need clear cancel, return, and already-paid fallback states.",
    },
    invite_sheet: {
    ...
    invite_actions: {
      ...
      systemEvents: [
        "Creates external invites or existing-user access requests",
      ],
    },
    ...
    join_org: {
      ...
      systemEvents: [
        "Completes external invite review and membership acceptance",
      ],
    },
    access_requests: {
      dataFields: [
    ...
    workspace_activation_outcomes: {
      dataFields: [
        ...
        "first invite or coaching action",
      ],
      systemEvents: [
        "Should turn workspace activity into measurable activation checkpoints",
      ],
      healthStatus: "activation-gap",
      healthSummary:
        "Add a durable activation model so product progress is not just isolated form saves.",
    },
    product_analytics: {
      dataFields: [
        "CTA clicked",
    ...
    otel_instrumentation: {
      ...
      systemEvents: [
        "Initializes server observability when an exporter is configured",
      ],
      healthStatus: "telemetry-gap",
      healthSummary:
        "Server traces are optional and not tied to user journey milestones.",
    },
    coaching_schedule: {
      dataFields: [
        "active org",
      ...
      systemEvents: [
        "Returns Google booking links and records that scheduling was opened",
      ],
      healthStatus: "integration-gap",
      healthSummary:
        "Scheduling opens an external booking link but does not sync confirmed meetings yet.",
    },
    coaching_tier_rules: {
      dataFields: [
        "included meetings",
        "meetings used",
        "free/discounted/full tier",
      ],
      systemEvents: ["Chooses the coaching booking tier for the account"],
      healthStatus: "integration-gap",
      healthSummary:
        "Eligibility is local, but confirmed appointments are not reconciled from calendars.",
    },
    notification_create: {
    ...
    homework_assist_library: {
      ...
      healthStatus: "ai-stub",
      healthSummary:
        "Replace the deterministic helper with a model-backed assistant and review loop.",
    },
  }
