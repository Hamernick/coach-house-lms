...
function statusCopy(
  status: ReturnType<
    typeof useWorkspaceBrandKitController
  >["readiness"]["status"]
) {
  if (status === "ready")
    return {
...
function AssetStatusRow({
  ...
  return (
    <div className="border-border/60 bg-background/40 flex items-center gap-3 rounded-2xl border px-3 py-2.5">
      ...
      {ready ? (
        <BadgeCheckIcon className="h-4 w-4 text-emerald-500" aria-hidden />
      ) : null}
    </div>
  )
}
...
function SegmentedTabs({
  ...
  return (
    <div className="bg-muted/70 inline-flex items-center rounded-full p-1">
      {tabs.map((tab) => {
...
export function WorkspaceBrandKitPanel({ input }: WorkspaceBrandKitPanelProps) {
  ...
  const brandVoice =
    controller.draftProfile.brandVoiceGuidelines?.trim() ||
    controller.draftProfile.brandVoiceTone?.trim() ||
    controller.draftProfile.brandVoicePersonality?.trim() ||
    ""
  ...
  const title =
    (controller.draftProfile.name ?? "").trim() ||
    "Set your organization basics"
  const subtitle =
      ...
      <div className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] gap-3">
        <div className="min-w-0 space-y-2.5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-muted-foreground text-[11px]">
              {controller.readiness.completedCount}/
              {controller.readiness.totalCount} essentials
            </span>
          </div>
        ...
        {activeTab === "overview" ? (
          <div className="grid min-h-0 grid-rows-[minmax(0,1fr)_auto] gap-3 overflow-hidden">
            <div className="border-border/60 bg-background/35 min-h-0 overflow-hidden rounded-2xl border p-3">
              <div className="flex h-full min-h-0 flex-col overflow-hidden">
                <p className="text-muted-foreground text-xs font-medium">
                  Brand voice
                </p>
                <p className="text-foreground mt-1 line-clamp-5 text-sm leading-relaxed">
                  {brandVoice ||
                    "Define the tone, audience, and language your team should use."}
                </p>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-2">
              <div className="border-border/60 bg-background/35 rounded-2xl border p-3">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {palette.length > 0 ? (
                    palette.map((color) => (
                      <span
                        key={color}
                        className="border-border/60 text-foreground inline-flex items-center gap-2 rounded-full border px-2 py-1 text-[11px]"
                      >
                        <span
                    ...
                    <p className="text-muted-foreground text-xs">
                      Choose a primary color and a few supporting colors.
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="grid min-h-0 grid-rows-[auto_auto_minmax(0,1fr)] gap-3 overflow-hidden">
            ...
            <div className="border-border/60 bg-background/35 rounded-2xl border p-3">
              ...
              <p className="text-foreground mt-1 line-clamp-3 text-sm leading-relaxed">
                SVG is preferred for logos. PNG, JPEG, and WebP are accepted
                across the kit. Use a wide primary logo, a square mark, and a
                wide banner image for profile headers.
              </p>
            </div>
            <div className="border-border/60 bg-muted/15 min-h-0 overflow-hidden rounded-2xl border border-dashed p-3">
              <p className="text-muted-foreground text-xs font-medium">
                Social connections
              </p>
              <p className="text-foreground mt-1 line-clamp-3 text-sm">
                Instagram, LinkedIn, X, Facebook, and newsletter sync are staged
                as coming soon.
              </p>
            </div>
          </div>
        ...
        <div className="border-border/60 grid grid-cols-2 gap-2 border-t pt-3">
          <Button
            ...
            size="sm"
            className="w-full"
            onClick={() => controller.setIsSheetOpen(true)}
          >
