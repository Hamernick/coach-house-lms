...
-- Ensure session S6 (budgets to bookkeeping) content is applied to the live class slug
update modules m
set content_md = $MD$
...
where m.class_id = c.id
  and c.slug = 'session-s6-budgets-to-bookkeeping'
  and m.slug = 'from-budgeting-to-bookkeeping';
...
where m.class_id = c.id
  and c.slug = 'session-s6-budgets-to-bookkeeping'
  and m.slug = 'financials-ie-bs-coa';

-- Fundraising homework for session S7 (mindset)
...
select
  ...
  case
    ...
    when fm.slug = 'segmentation' then
      jsonb_build_object(
        'title', 'Segmentation',
        'fields', jsonb_build_array(
          jsonb_build_object(
    ...
    when fm.slug = 'storytelling-and-the-ask' then
      jsonb_build_object(
    ...
    when fm.slug = 'tools-and-systems' then
      jsonb_build_object(
...
-- Communications homework for session S8 (comms as mission)
with comm_modules as (
  select m.id, m.slug
  from classes c
  join modules m on m.class_id = c.id
  where c.slug = 'session-s8-comms-as-mission'
    and m.slug in ('comms-as-mission', 'comprehensive-plan')
)
insert into module_assignments (module_id, schema, complete_on_submit)
select
  cm.id,
  case
    when cm.slug = 'comms-as-mission' then
      jsonb_build_object(
