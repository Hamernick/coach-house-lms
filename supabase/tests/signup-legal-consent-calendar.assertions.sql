...
begin
  for version_data in select value from jsonb_array_elements('[
    ...
    fixture_id := ('00000000-0000-0000-0000-' || lpad((200 + fixture_index)::text, 12, '0'))::uuid;
    ...
    values (fixture_id, jsonb_build_object('legal_consent', consent), null, '2026-09-09T15:00:00Z');
    ...
    if acceptance.document_version <> consent ->> 'version'
      ...
      or acceptance.accepted_at <> '2026-09-09T15:00:00Z'::timestamptz
      or acceptance.source <> 'signup'
    then
      raise exception 'Calendar rollout failed consent tuple or server timestamp: %', fixture_index;
    end if;
  end loop;

  -- A new version must not accept old document hashes, and vice versa.
  for consent in select value from jsonb_array_elements('[
    {"version":"2026-09-09.1","termsSha256":"51bde17c17824786259ae9fe35f5b0740c7638c4705795d3de2805d1d0d80220","privacySha256":"4110a62f34947f3950dc75b7e6ffab87f60d9abfe48b44625c8150da5e845e62","acceptedAt":"2026-09-09T15:00:00Z"},
    {"version":"2026-08-27.1","termsSha256":"7976c515ea20f08a89c0334d4135733d7a0d1088780bb1e6d5768d1c7ababef0","privacySha256":"3ba370741180467b5e1b43fad91feb6b0ac393634e500487588d15951e069b9f","acceptedAt":"2026-09-09T15:00:00Z"}
  ]'::jsonb) loop
    begin
      insert into auth.users (id, raw_user_meta_data, raw_app_meta_data)
      values ('00000000-0000-0000-0000-000000000299', jsonb_build_object('legal_consent', consent), '{}'::jsonb);
      raise exception 'Mismatched consent version and hashes were accepted';
    exception when sqlstate '22023' then null;
    end;
  end loop;

  if exists (select 1 from auth.users where id = '00000000-0000-0000-0000-000000000299') then
  ...
  if (select count(*) from public.platform_legal_acceptances
    ...
    raise exception 'Calendar rollout modified historical acceptance';
  end if;
end;
$$;
