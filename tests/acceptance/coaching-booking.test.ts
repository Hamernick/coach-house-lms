...
describe("coaching booking feature", () => {
  it("adds coaching as a dashboard route backed by a feature slice", () => {
    ...
    expect(flow).toContain("Book a 45-minute advisory meeting with Coach House leadership")
    ...
    expect(flow).toContain('aria-label={detailsExpanded ? "Show less meeting details" : "Show more meeting details"}')
    ...
    expect(participantStacks).toContain('import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip"')
    expect(participantStacks).toContain("function getFirstName(name: string)")
    expect(participantStacks).toContain("return trimmedName.split(/\\s+/)[0] ?? trimmedName")
    ...
    expect(participantStacks).toContain('<TooltipContent side="top" sideOffset={8} className="text-xs font-medium">')
    ...
    expect(flow).toContain("flex min-h-full flex-1 items-stretch justify-start px-0 py-0 sm:items-center sm:justify-center sm:px-4 sm:py-6 lg:px-6")
    ...
    expect(participantStacks).toContain("border-background size-10 min-h-10 min-w-10 max-h-10 max-w-10 basis-10 border-2")
    expect(participantStacks).toContain("border-background size-9 min-h-9 min-w-9 max-h-9 max-w-9 basis-9 border-2")
    expect(participantStacks).toContain("outline-none hover:z-10 focus-visible:z-10")
    expect(participantStacks).toContain("focus-visible:ring-2 focus-visible:ring-ring")
    ...
    expect(participantStacks).toContain('const COACHING_REVIEW_AVATAR_FALLBACK_CLASS = "size-full text-[11px] font-medium leading-none"')
    ...
    expect(flow).toContain("Pick an open meeting above to review the meeting details.")
    ...
    expect(flow).toContain('className="flex flex-col items-center gap-4 text-center"')
    ...
    expect(participantStacks).toContain("avatarClassName={COACHING_REVIEW_AVATAR_CLASS}")
    ...
    expect(flow).not.toContain("{COACHING_JOINT_COACH_LABEL} · {formatDateTime(selectedSlot.startsAt, timezone)}")
    ...
    expect(flow).toContain("Share priorities, questions, or context for the meeting…")
    ...
    expect(flow).toContain("Calendar availability is unavailable in this local environment.")
    expect(participantStacks).toContain("person.imageUrl ?")
    expect(participantStacks).toContain("className={COACHING_AVATAR_IMAGE_CLASS}")
    expect(participantStacks).toContain("fallbackClassName={COACHING_REVIEW_AVATAR_FALLBACK_CLASS}")
    ...
    expect(appShell).toContain('pathname === "/coaching" || Boolean(pathname?.startsWith("/coaching/"))')
    ...
    expect(appShell).toContain("useFullBleedContent || useMobileSingleGutterContent")
    expect(shellMainContent).toContain('import { ScrollFadeEffect } from "@/components/scroll-fade-effect"')
    expect(shellMainContent).toContain("<ScrollFadeEffect")
    expect(shellMainContent).toContain("enabled={useMobileSingleGutterContent}")
    expect(shellMainContent).toContain("[--mask-height:2rem] [--scroll-buffer:1.5rem]")
  ...
  it("adds DB-backed bookings, credits, RLS, and Stripe webhook handoff", () => {
    ...
    expect(actions).not.toContain("resolveStripeRuntimeConfigForCoaching({ isTester: audience.isTester")
    ...
    expect(finalizer).toContain("Coach House meeting with ${COACHING_JOINT_COACH_LABEL}")
    ...
    expect(icsRoute).toContain('const CANONICAL_SITE_URL = "https://coachhouse.app"')
    ...
    expect(icsRoute).toContain('const ICS_FILENAME_PREFIX = "coach-house-advisory-meeting"')
