...
describe("people table controls", () => {
  ...
  it("uses one scrollable person edit form instead of a stepped sheet", () => {
    ...
    expect(dialogSource).toContain("Person details")
    expect(dialogSource).toContain("readOnly={readOnly}")
    expect(fieldsSource).toContain("readOnly={readOnly}")
    expect(fieldsSource).toContain("disabled={readOnly}")
    expect(fieldsSource).toContain("Profile")
    expect(fieldsSource).toContain("Contact")
    expect(fieldsSource).toContain("Social media")
    expect(fieldsSource).toContain("PERSON_SOCIAL_PLATFORMS.map")
    ...
    expect(dialogSource).toContain("...submittedSocialLinks")
