...
describe("workspace-brand-kit", () => {
  ...
  it("uses structured Brand Voice across saved and public Brand Kit surfaces", () => {
    const root = process.cwd()
    const panel = readFileSync(
      join(
        root,
        "src/features/workspace-brand-kit/components/workspace-brand-kit-panel.tsx"
      ),
      "utf8"
    )
    const preview = readFileSync(
      join(
        root,
        "src/features/workspace-brand-kit/components/workspace-brand-kit-profile-preview.tsx"
      ),
      "utf8"
    )
    const displaySections = readFileSync(
      join(
        root,
    ...
    const publicCard = readFileSync(
      join(
        root,
        "src/components/organization/org-profile-card/public-card.tsx"
      ),
      "utf8"
    )
    const profileCleanup = readFileSync(
      join(root, "src/lib/organization/profile-cleanup.ts"),
      "utf8"
    )
    const profileConfig = readFileSync(
      join(root, "src/components/organization/org-profile-card/config.ts"),
      "utf8"
    )

    expect(panel).toContain("draftProfile.brandVoiceGuidelines")
    expect(panel).not.toContain("draftProfile.boilerplate")
    expect(preview).toContain("WorkspaceBrandKitProfilePreview")
    expect(preview).toContain("BrandVoicePreview")
    expect(displaySections).toContain("<WorkspaceBrandKitProfilePreview")
    expect(publicCard).not.toContain("Boilerplate")
    expect(publicCard).not.toContain("showBoilerplateSeparator")
    expect(profileCleanup).toContain('"brandVoiceAudience"')
    expect(profileCleanup).toContain('"brandVoiceGuidelines"')
